...
import (
	...
	"cpa-plugin-account-usage/internal/plugin"
)

type cgoHostCaller struct{}

var app = plugin.NewApp(cgoHostCaller{})
...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	if errHandle != nil {
		writeResponse(response, plugin.ErrorEnvelope("plugin_error", errHandle.Error(), 500))
		return 1
	}
...
func (cgoHostCaller) Call(method string, payload []byte) ([]byte, error) {
	...
	if callCode != 0 {
		return rawResponse, fmt.Errorf("host callback %s returned code=%d", method, int(callCode))
	}
